...
// --- 使用者設定 ---
// WiFi 設定已移至 setup() 使用 wifiMulti.addAP 支援多組熱點
...
void setup() {
    ...
    // 2. WiFi 連線效能優化與多組 WiFi 容錯設定
    Serial.println("[系統日誌] 步驟 2: 配置多組 WiFi 熱點並關閉無線休眠...");
    WiFi.setSleep(false); // [關鍵優化] 關閉 WiFi 休眠以避免 RTSP 延遲與封包遺失
    
    // 添加多組 WiFi 帳號密碼 (程式會自動掃描並連線訊號最好的)
    wifiMulti.addAP("maggie888", "aaaa222288");
    wifiMulti.addAP("win1788", "88888888");
    wifiMulti.addAP("win8888", "88888888");

    Serial.print("[WiFi 日誌] 自動搜尋並等待 WiFi 連線 ");
